...
public static class HttpExtensions
{
    ...
    [System.Diagnostics.CodeAnalysis.SuppressMessage("Style", "IDE0028", Justification = "ConditionalWeakTable does not support collection expressions on net4x.")]
    private static readonly ConditionalWeakTable<IBlobCache, IHttpService> HttpServices = new();

    /// <summary>
    /// Associates an <see cref="IHttpService"/> with a cache instance so that
    /// <see cref="DownloadUrl(IBlobCache, string, HttpMethod?, IEnumerable{KeyValuePair{string, string}}?, bool, DateTimeOffset?)"/>
    /// and related extensions can locate the service.
    /// </summary>
    ...
    public static void SetHttpService(this IBlobCache blobCache, IHttpService httpService)
    {
        ArgumentExceptionHelper.ThrowIfNull(blobCache);
        ArgumentExceptionHelper.ThrowIfNull(httpService);

        HttpServices.Remove(blobCache);
    ...
    /// <summary>
    /// Gets the HTTP service associated with a cache, creating a default one if none was set.
    /// </summary>
