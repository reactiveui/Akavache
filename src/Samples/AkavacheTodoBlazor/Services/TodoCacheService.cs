...
/// <summary>
/// Service that demonstrates comprehensive Akavache usage for the Blazor Todo application.
/// </summary>
public class TodoCacheService
{
    ...
    public IObservable<Unit> SaveSettings(AppSettings settings)
    {
        ...
        // Save to UserAccount for settings that should persist across sessions
    ...
    /// <summary>
    /// Demonstrates session-based caching using InMemory cache.
    /// </summary>
    /// <param name="sessionData">The session data to cache.</param>
    /// <returns>Observable unit.</returns>
    public IObservable<Unit> SaveSessionData(Dictionary<string, object> sessionData)
    {
        // Use InMemory cache for session data that should not persist
        return BlobCache.InMemory.InsertObject(SessionDataKey, sessionData, DateTimeOffset.Now.AddHours(1));
    }

    /// <summary>
    /// Gets session data from memory cache.
    /// </summary>
    /// <returns>Observable session data.</returns>
    public IObservable<Dictionary<string, object>> GetSessionData()
    {
        return BlobCache.InMemory
            .GetObject<Dictionary<string, object>>(SessionDataKey)
    ...
    public IObservable<CacheInfo> GetCacheInfo() => Observable.CombineLatest(
            BlobCache.UserAccount.GetAllKeys(),
            BlobCache.LocalMachine.GetAllKeys(),
            BlobCache.Secure.GetAllKeys(),
            BlobCache.InMemory.GetAllKeys(),
            (userKeys, localKeys, secureKeys, memoryKeys) => new CacheInfo
            {
                UserAccountKeys = userKeys.Length,
                LocalMachineKeys = localKeys.Length,
                SecureKeys = secureKeys.Length,
                InMemoryKeys = memoryKeys.Length,
                TotalKeys = userKeys.Length + localKeys.Length + secureKeys.Length + memoryKeys.Length,
                LastChecked = DateTimeOffset.Now
            });

    /// <summary>
    /// Simulates API data fetching with caching.
    /// </summary>
    /// <param name="endpoint">The API endpoint.</param>
    /// <returns>Observable API response.</returns>
    public IObservable<ApiResponse> GetApiData(string endpoint)
    {
        var cacheKey = $"api_{endpoint}";

        return BlobCache.LocalMachine.GetAndFetchLatest(
            cacheKey,
            () => Observable.FromAsync(() => SimulateApiCall(endpoint)),
            fetchPredicate: createdAt => DateTimeOffset.Now - createdAt > TimeSpan.FromMinutes(10),
    ...
    private async Task<ApiResponse> SimulateApiCall(string endpoint)
    {
