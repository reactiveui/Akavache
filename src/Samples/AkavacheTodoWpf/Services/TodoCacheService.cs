...
public class TodoCacheService
{
    ...
    public static IObservable<Dictionary<string, DateTimeOffset?>> GetTodoMetadata(IEnumerable<string> todoIds)
    {
    ...
    /// <returns>Observable unit.</returns>
    public static IObservable<Unit> SaveTemporaryData(object data, int expirationMinutes = 5)
    {
    ...
    /// </summary>
    /// <returns>Observable unit.</returns>
    public static IObservable<Unit> CleanupCache() => BlobCache.LocalMachine.Vacuum().Merge(
    ...
    public static IObservable<List<string>> GetAllCacheKeys() => BlobCache.UserAccount
            .GetAllKeys()
    ...
    /// <returns>Observable unit.</returns>
    public static IObservable<Unit> SaveSecureData(Dictionary<string, string> userCredentials) =>
        BlobCache.Secure.InsertObject("user_credentials", userCredentials);

    /// <summary>
    /// Gets secure data from encrypted cache.
    /// </summary>
    /// <returns>Observable dictionary of user credentials.</returns>
    public static IObservable<Dictionary<string, string>?> GetSecureData() => BlobCache.Secure
    ...
    public static IObservable<CacheInfo> GetCacheInfo() => BlobCache.UserAccount.GetAllKeys().CombineLatest(
    ...
    public static IObservable<AppState?> LoadApplicationState() => BlobCache.UserAccount
    ...
    public static IObservable<TodoItem?> GetTodo(string todoId)
    {
        var key = $"todo_{todoId}";
        return BlobCache.UserAccount.GetOrFetchObject(
            key, async () =>
            {
    ...
    public static IObservable<TodoStats?> GetTodoStats() => BlobCache.LocalMachine.GetAndFetchLatest(
            CacheStatsKey,
    ...
    private static async Task<TodoStats> CalculateTodoStats()
    {
        var todos = await GetAllTodos().FirstAsync();
