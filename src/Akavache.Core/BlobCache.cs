...
namespace Akavache
{
    ...
    public static class BlobCache
    {
        ...
        /// <summary>
        /// The local machine cache. Store data here that is unrelated to the user account or
        /// shouldn't be uploaded to other machines (i.e. image cache data)
        /// </summary>
        public static IBlobCache LocalMachine
        {
            get
            {
                return unitTestLocalMachine ?? localMachine ?? (shutdownRequested ? new ShutdownBlobCache() : null) ?? Locator.Current.GetService<IBlobCache>("LocalMachine");
            }
        ...
        /// <summary>
        /// The user account cache. Store data here that is associated with the user; in large
        /// organizations, this data will be synced to all machines via NT Roaming Profiles.
        /// </summary>
        public static IBlobCache UserAccount
        {
            get
            {
                return unitTestUserAccount ?? userAccount ?? (shutdownRequested ? new ShutdownBlobCache() : null) ?? Locator.Current.GetService<IBlobCache>("UserAccount");
            }
        ...
        /// <summary>
        /// An IBlobCache that is encrypted - store sensitive data in this cache such as login information.
        /// </summary>
        public static ISecureBlobCache Secure
        {
            get
            {
                return unitTestSecure ?? secure ?? (shutdownRequested ? new ShutdownBlobCache() : null) ?? Locator.Current.GetService<ISecureBlobCache>();
            }
        ...
        /// <summary>
        /// An IBlobCache that simply stores data in memory. Data stored in this cache will be lost
        ...
        /// <remarks>
        /// <para>
        /// By default, <see cref="BsonReader"/> uses a <see cref="DateTimeKind"/> of <see
        /// cref="DateTimeKind.Local"/> and <see cref="BsonWriter"/> uses <see
        /// cref="DateTimeKind.Utc"/>. Thus, DateTimes are serialized as UTC but deserialized as
        /// local time. To force BSON readers to use some other <c>DateTimeKind</c>, you can set this value.
        /// </para>
        /// </remarks>
        ...
        {
            // NB: This method doesn't actually do anything, it just ensures that the static
            // constructor runs
            LogHost.Default.Debug("Initializing Akavache");
        }

        /// <summary>
        /// This method shuts down all of the blob caches. Make sure call it on app exit and await /
        /// Wait() on it!
        /// </summary>
        /// <returns>A Task representing when all caches have finished shutting down.</returns>
