...
namespace Akavache
{
    public static class JsonSerializationMixin
    {
        ...
        /// <summary>
        /// Insert several objects into the cache, via the JSON serializer. Similarly to InsertAll,
        /// partial inserts should not happen.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="This">The this.</param>
        ...
        public static IObservable<IEnumerable<T>> GetAllObjects<T>(this IBlobCache This)
        {
            ...
            // NB: This isn't exactly thread-safe, but it's Close Enough(tm) We make up for the fact
            // that the keys could get kicked out from under us via the Catch below
        ...
        /// <summary>
        /// Attempt to return an object from the cache. If the item doesn't exist or returns an
        /// error, call a Func to return the latest version of an object and insert the result in the
        /// cache. For most Internet applications, this method is the best method to call to fetch
        /// static data (i.e. images) from the network.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="This">The this.</param>
        /// <param name="key">The key to associate with the object.</param>
        /// <param name="fetchFunc">
        /// A Func which will asynchronously return the latest value for the object should the cache
        /// not contain the key. Observable.Start is the most straightforward way (though not the
        /// most efficient!) to implement this Func.
        /// </param>
        /// <param name="absoluteExpiration">An optional expiration date.</param>
        /// <returns>A Future result representing the deserialized object from the cache.</returns>
        ...
        /// <summary>
        /// Attempt to return an object from the cache. If the item doesn't exist or returns an
        /// error, call a Func to create a new one. For most Internet applications, this method is
        /// the best method to call to fetch static data (i.e. images) from the network.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="This">The this.</param>
        /// <param name="key">The key to associate with the object.</param>
        ...
        /// <summary>
        /// This method attempts to returned a cached value, while simultaneously calling a Func to
        /// return the latest value. When the latest data comes back, it replaces what was previously
        /// in the cache. This method is best suited for loading dynamic data from the Internet,
        /// while still showing the user earlier data. This method returns an IObservable that may
        /// return *two* results (first the cached data, then the latest data). Therefore, it's
        /// important for UI applications that in your Subscribe method, you write the code to merge
        /// the second result when it comes in. This also means that await'ing this method is a Bad
        /// Idea(tm), always use Subscribe.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="This">The this.</param>
        /// <param name="key">The key to store the returned result under.</param>
        /// <param name="fetchFunc">The fetch function.</param>
        /// <param name="fetchPredicate">
        /// An optional Func to determine whether the updated item should be fetched. If the cached
        /// version isn't found, this parameter is ignored and the item is always fetched.
        /// </param>
        /// <param name="absoluteExpiration">An optional expiration date.</param>
        /// <param name="shouldInvalidateOnError">
        /// If this is true, the cache will be cleared when an exception occurs in fetchFunc
        /// </param>
        /// <returns>
        /// An Observable stream containing either one or two results (possibly a cached version,
