...
public class SqliteBlobCacheDirectTests
{
    ...
    public async Task DisposedShouldThrowForAllOperations()
    {
        using (Utility.WithEmptyDirectory(out var path))
        {
            ...
            await Assert.That(async () => await cache.Insert("k", new byte[] { 1 }).ToTask()).Throws<ObjectDisposedException>();
            await Assert.That(async () => await cache.Insert(new[] { new KeyValuePair<string, byte[]>("k", new byte[] { 1 }) }).ToTask()).Throws<ObjectDisposedException>();
            await Assert.That(async () => await cache.Insert("k", new byte[] { 1 }, typeof(string)).ToTask()).Throws<ObjectDisposedException>();
            await Assert.That(async () => await cache.Insert(new[] { new KeyValuePair<string, byte[]>("k", new byte[] { 1 }) }, typeof(string)).ToTask()).Throws<ObjectDisposedException>();

            await Assert.That(async () => await cache.Get("k").ToTask()).Throws<ObjectDisposedException>();
            await Assert.That(async () => await cache.Get(new[] { "k" }).ToList().ToTask()).Throws<ObjectDisposedException>();
            await Assert.That(async () => await cache.Get("k", typeof(string)).ToTask()).Throws<ObjectDisposedException>();
            await Assert.That(async () => await cache.Get(new[] { "k" }, typeof(string)).ToList().ToTask()).Throws<ObjectDisposedException>();
            ...
            await Assert.That(async () => await cache.GetCreatedAt(new[] { "k" }).ToList().ToTask()).Throws<ObjectDisposedException>();
            await Assert.That(async () => await cache.GetCreatedAt("k", typeof(string)).ToTask()).Throws<ObjectDisposedException>();
            await Assert.That(async () => await cache.GetCreatedAt(new[] { "k" }, typeof(string)).ToList().ToTask()).Throws<ObjectDisposedException>();
            ...
            await Assert.That(async () => await cache.Invalidate(new[] { "k" }).ToTask()).Throws<ObjectDisposedException>();
            await Assert.That(async () => await cache.Invalidate("k", typeof(string)).ToTask()).Throws<ObjectDisposedException>();
            await Assert.That(async () => await cache.Invalidate(new[] { "k" }, typeof(string)).ToTask()).Throws<ObjectDisposedException>();
            ...
            await Assert.That(async () => await cache.UpdateExpiration(new[] { "k" }, DateTimeOffset.Now).ToTask()).Throws<ObjectDisposedException>();
            ...
            await Assert.That(async () => await cache.UpdateExpiration(new[] { "k" }, typeof(string), DateTimeOffset.Now).ToTask()).Throws<ObjectDisposedException>();
        }
    }
    ...
    public async Task NullArgsShouldThrow()
    {
        using (Utility.WithEmptyDirectory(out var path))
        {
            ...
            try
            {
                ...
                await Assert.That(async () => await cache.Get(new[] { "k" }, (Type)null!).ToList().ToTask()).Throws<ArgumentNullException>();
    ...
    public async Task TypeAwareInsertAndGetShouldRoundTrip()
    {
        using (Utility.WithEmptyDirectory(out var path))
        {
            ...
            try
            {
                await cache.Insert("k1", new byte[] { 1, 2, 3 }, typeof(string)).ToTask();
    ...
    public async Task TypeAwareBulkInsertAndGetShouldRoundTrip()
    {
        using (Utility.WithEmptyDirectory(out var path))
        {
            ...
            try
            {
                ...
                var results = await cache.Get(new[] { "k1", "k2" }, typeof(string)).ToList().ToTask();
    ...
    public async Task TypeAwareInvalidateShouldRemoveEntries()
    {
        using (Utility.WithEmptyDirectory(out var path))
        {
            ...
            try
            {
                ...
                await cache.Insert("k2", new byte[] { 2 }, typeof(int)).ToTask();
    ...
    public async Task TypeAwareInvalidateByKeysShouldRemoveEntries()
    {
        using (Utility.WithEmptyDirectory(out var path))
        {
            ...
            try
            {
                ...
                await cache.Invalidate(new[] { "k1", "k2" }, typeof(string)).ToTask();
    ...
    public async Task TypeAwareGetCreatedAtShouldReturnTimestamps()
    {
        using (Utility.WithEmptyDirectory(out var path))
        {
            ...
            try
            {
                ...
                var multi = await cache.GetCreatedAt(new[] { "k1", "k2" }, typeof(string)).ToList().ToTask();
    ...
    public async Task TypeAwareUpdateExpirationShouldUpdateEntries()
    {
        using (Utility.WithEmptyDirectory(out var path))
        {
            ...
            try
            {
                ...
                await cache.UpdateExpiration(new[] { "k1" }, typeof(string), newExpiration).ToTask();
    ...
    public async Task VacuumShouldWork()
    {
        using (Utility.WithEmptyDirectory(out var path))
        {
            ...
            try
            {
                await cache.Insert("k1", new byte[] { 1 }, DateTimeOffset.Now.AddSeconds(-10)).ToTask();
                await cache.Insert("k2", new byte[] { 2 }, DateTimeOffset.Now.AddHours(1)).ToTask();
    ...
    public async Task NonTypedHappyPathsShouldRoundTrip()
    {
        using (Utility.WithEmptyDirectory(out var path))
        {
            ...
            try
            {
                await cache.Insert("k1", new byte[] { 1, 2 }).ToTask();
                await cache.Insert(
                    new[]
                    {
                        new KeyValuePair<string, byte[]>("k2", new byte[] { 3 }),
                        new KeyValuePair<string, byte[]>("k3", new byte[] { 4 }),
                    },
                ...
                var multi = await cache.Get(new[] { "k2", "k3" }).ToList().ToTask();
                await Assert.That(multi.Count).IsEqualTo(2);
    ...
    public async Task AdditionalNullAndWhitespaceArgsShouldThrow()
    {
        using (Utility.WithEmptyDirectory(out var path))
        {
            ...
            try
            {
                ...
                await Assert.That(async () => await cache.GetCreatedAt(new[] { "k" }, (Type)null!).ToList().ToTask()).Throws<ArgumentNullException>();
                ...
                await Assert.That(async () => await cache.Insert(new[] { new KeyValuePair<string, byte[]>("k", new byte[] { 1 }) }, (Type)null!).ToTask()).Throws<ArgumentNullException>();

                // Insert(key, data, type) arg validation
                await Assert.That(async () => await cache.Insert(string.Empty, new byte[] { 1 }, typeof(string)).ToTask()).Throws<ArgumentException>();
                await Assert.That(async () => await cache.Insert("  ", new byte[] { 1 }, typeof(string)).ToTask()).Throws<ArgumentException>();
                await Assert.That(async () => await cache.Insert("k", (byte[])null!, typeof(string)).ToTask()).Throws<ArgumentNullException>();
                await Assert.That(async () => await cache.Insert("k", new byte[] { 1 }, (Type)null!).ToTask()).Throws<ArgumentNullException>();
                ...
                await Assert.That(async () => await cache.Invalidate(new[] { "k" }, (Type)null!).ToTask()).Throws<ArgumentNullException>();
                ...
                await Assert.That(async () => await cache.UpdateExpiration(new[] { "k" }, (Type)null!, DateTimeOffset.Now).ToTask()).Throws<ArgumentNullException>();
            }
            finally
            {
                await cache.DisposeAsync();
    ...
    public async Task InsertWithPastExpirationShouldNotBeRetrievable()
    {
        using (Utility.WithEmptyDirectory(out var path))
        {
            ...
            try
            {
                await cache.Insert("expired", new byte[] { 1 }, typeof(string), DateTimeOffset.UtcNow.AddDays(-1)).ToTask();
    ...
    public async Task InMemoryBulkUpdateExpirationShouldMutateAllEntries()
    {
        ...
        try
        {
            ...
            await cache.UpdateExpiration(new[] { "k1", "k2", "k3" }, updated).ToTask();
    ...
    public async Task InMemoryBulkUpdateExpirationWithTypeShouldRespectTypeFilter()
    {
        ...
        try
        {
            var originalExpiry = DateTimeOffset.UtcNow.AddMinutes(1).UtcDateTime;
            ...
            await cache.UpdateExpiration(new[] { "a", "b" }, typeof(string), updated).ToTask();
            ...
            await cache.UpdateExpiration(new[] { "a", "b" }, typeof(int), wrongTypeExpiry).ToTask();
            await Assert.That(connection.Store["a"].ExpiresAt!.Value).IsEqualTo(updated.UtcDateTime);
            await Assert.That(connection.Store["b"].ExpiresAt!.Value).IsEqualTo(updated.UtcDateTime);
        }
        finally
    ...
    public async Task InMemoryInvalidateWithTypeShouldOnlyRemoveTypedEntries()
    {
        ...
        try
        {
            ...
            await cache.Invalidate(new[] { "a", "b" }, typeof(string)).ToTask();
