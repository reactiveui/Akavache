...
public class Sqlite3SecureBlobCacheWrapperTests
{
    ...
    public async Task ShouldInsertAndGet()
    {
        ...
        try
        {
            await wrapper.Insert("k", new byte[] { 1, 2, 3 }).ToTask();
    ...
    public async Task ShouldInsertAndGetMultiple()
    {
        ...
        try
        {
            ...
            var results = await wrapper.Get(new[] { "k1", "k2" }).ToList().ToTask();
    ...
    public async Task ShouldInsertAndGetWithType()
    {
        ...
        try
        {
            await wrapper.Insert("k", new byte[] { 1 }, typeof(string)).ToTask();
    ...
    public async Task ShouldInsertMultipleWithType()
    {
        ...
        try
        {
            ...
            var results = await wrapper.Get(new[] { "k1", "k2" }, typeof(string)).ToList().ToTask();
            await Assert.That(results.Count).IsEqualTo(2);
    ...
    public async Task ShouldGetCreatedAt()
    {
        ...
        try
        {
            await wrapper.Insert("k1", new byte[] { 1 }).ToTask();
            await wrapper.Insert("k2", new byte[] { 2 }).ToTask();

            var single = await wrapper.GetCreatedAt("k1").ToTask();
            await Assert.That(single).IsNotNull();

            var multi = await wrapper.GetCreatedAt(new[] { "k1", "k2" }).ToList().ToTask();
            await Assert.That(multi.Count).IsEqualTo(2);

            await wrapper.Insert("k3", new byte[] { 3 }, typeof(int)).ToTask();
            var typed = await wrapper.GetCreatedAt("k3", typeof(int)).ToTask();
            await Assert.That(typed).IsNotNull();

            var typedMulti = await wrapper.GetCreatedAt(new[] { "k3" }, typeof(int)).ToList().ToTask();
    ...
    public async Task ShouldInvalidate()
    {
        ...
        try
        {
            await wrapper.Insert("k1", new byte[] { 1 }).ToTask();
            await wrapper.Insert("k2", new byte[] { 2 }).ToTask();
            await wrapper.Insert("k3", new byte[] { 3 }, typeof(string)).ToTask();
            await wrapper.Insert("k4", new byte[] { 4 }, typeof(int)).ToTask();

            await wrapper.Invalidate("k1").ToTask();
            await wrapper.Invalidate("k3", typeof(string)).ToTask();
            await wrapper.Invalidate(new[] { "k2" }).ToTask();
            await wrapper.Invalidate(new[] { "k4" }, typeof(int)).ToTask();
    ...
    public async Task ShouldInvalidateAll()
    {
        ...
        try
        {
            await wrapper.Insert("k1", new byte[] { 1 }, typeof(string)).ToTask();
    ...
    public async Task ShouldUpdateExpiration()
    {
        ...
        {
            await wrapper.Insert("k1", new byte[] { 1 }).ToTask();
            await wrapper.Insert("k2", new byte[] { 2 }, typeof(string)).ToTask();
            ...
            await wrapper.UpdateExpiration(new[] { "k1" }, future).ToTask();
            await wrapper.UpdateExpiration(new[] { "k2" }, typeof(string), future).ToTask();
        }
        finally
        {
            await wrapper.DisposeAsync();
    ...
    private static Sqlite3Extensions.SecureBlobCacheWrapper CreateWrapper() =>
        new(new InMemoryBlobCache(System.Reactive.Concurrency.ImmediateScheduler.Instance, new SystemJsonSerializer()));
}
