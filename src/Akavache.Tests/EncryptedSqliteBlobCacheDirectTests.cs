...
public class EncryptedSqliteBlobCacheDirectTests
{
    ...
    public async Task AdditionalNullArgsShouldThrow()
    {
        using (Utility.WithEmptyDirectory(out var path))
        {
            ...
            try
            {
                ...
                await Assert.That(async () => await cache.Insert(new[] { new KeyValuePair<string, byte[]>("k", [1]) }, (Type)null!).ToTask()).Throws<ArgumentNullException>();
    ...
    public async Task GetCreatedAtNonTypedHappyPath()
    {
        using (Utility.WithEmptyDirectory(out var path))
        {
            ...
            try
            {
                ...
                var multi = await cache.GetCreatedAt(new[] { "k1", "k2" }).ToList().ToTask();
                await Assert.That(multi.Count).IsEqualTo(2);
            }
            finally
            {
    ...
    public async Task UpdateExpirationNonTypedHappyPath()
    {
        using (Utility.WithEmptyDirectory(out var path))
        {
            ...
            try
            {
                ...
                await cache.UpdateExpiration(new[] { "k1", "k2" }, newExpiration).ToTask();
    ...
    public async Task GetBulkWithMissingKeysReturnsEmpty()
    {
        using (Utility.WithEmptyDirectory(out var path))
        {
            ...
            try
            {
                var results = await cache.Get(new[] { "missing1", "missing2" }).ToList().ToTask();
                await Assert.That(results).IsEmpty();

                var typedResults = await cache.Get(new[] { "missing1", "missing2" }, typeof(string)).ToList().ToTask();
    ...
    public async Task GetCreatedAtForMissingKeyReturnsDefault()
    {
        using (Utility.WithEmptyDirectory(out var path))
        {
            ...
            try
            {
                ...
                var multi = await cache.GetCreatedAt(new[] { "missing" }).ToList().ToTask();
