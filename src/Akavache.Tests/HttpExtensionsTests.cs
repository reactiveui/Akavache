...
public class HttpExtensionsTests
{
    ...
    public async Task WriteAsyncRxShouldThrowOnNullStream() =>
        await Assert.That(static () => HttpExtensions.WriteAsyncRx(null!, new byte[] { 1, 2, 3 }, 0, 3))
    ...
    public async Task WriteAsyncRxShouldPropagateException()
    {
        ...
        await Assert.That(async () => await stream.WriteAsyncRx(new byte[] { 1 }, 0, 1).ToTask())
            .Throws<Exception>();
    }
    ...
    {
        var cache = new InMemoryBlobCache(System.Reactive.Concurrency.ImmediateScheduler.Instance, new SystemJsonSerializer());
        try
        {
    ...
    {
        using var stream = new EndWriteThrowingStream();
