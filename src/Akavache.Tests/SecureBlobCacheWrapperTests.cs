...
public class SecureBlobCacheWrapperTests
{
    ...
    public async Task ShouldInsertAndGet()
    {
        ...
        try
        {
            await cache.Insert("k", new byte[] { 1, 2, 3 }).ToTask();
    ...
    public async Task ShouldInsertAndGetMultiple()
    {
        ...
        try
        {
            ...
            var results = await cache.Get(new[] { "k1", "k2" }).ToList().ToTask();
    ...
    public async Task ShouldGetCreatedAt()
    {
        ...
        try
        {
            ...
            await cache.Insert("k3", new byte[] { 3 }, typeof(int)).ToTask();
            var typed = await cache.GetCreatedAt("k3", typeof(int)).ToTask();
            await Assert.That(typed).IsNotNull();

            var typedMulti = await cache.GetCreatedAt(new[] { "k3" }, typeof(int)).ToList().ToTask();
    ...
    public async Task ShouldInvalidate()
    {
        ...
        try
        {
            await cache.Insert("k1", new byte[] { 1 }).ToTask();
            await cache.Insert("k2", new byte[] { 2 }).ToTask();
            await cache.Insert("k3", new byte[] { 3 }, typeof(string)).ToTask();
            await cache.Insert("k4", new byte[] { 4 }, typeof(int)).ToTask();

            await cache.Invalidate("k1").ToTask();
            await cache.Invalidate("k3", typeof(string)).ToTask();
            await cache.Invalidate(new[] { "k2" }).ToTask();
            await cache.Invalidate(new[] { "k4" }, typeof(int)).ToTask();
    ...
    public async Task ShouldUpdateExpiration()
    {
        ...
        try
        {
            ...
            await cache.UpdateExpiration(new[] { "k1" }, future).ToTask();
            await cache.UpdateExpiration(new[] { "k2" }, typeof(string), future).ToTask();
        }
        finally
        {
            await cache.DisposeAsync();
